...
def crawl(s, procnum, return_dict):
    USER_AGENT_LIST = [
...
if __name__ == '__main__':
    ...
    jobs = []
    for i in range(20):
        p = Process(target=crawl, args=(i*10000, i, return_dict))
        jobs.append(p)
        p.start()
    for proc in jobs:
        proc.join()
    return_dict = [pd.concat(return_dict[i]) for i in range(len(return_dict))]
    #results = pd.concat(return_dict)
    df_results = pd.concat(return_dict)
    '''
    ...
    df_results.to_csv('test_10000.csv', encoding='utf_8_sig', mode='a')
